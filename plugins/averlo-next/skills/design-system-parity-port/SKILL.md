...
description: Explicit-only Averlo workflow for building a page against an authoritative visual reference, freezing the matched renderer, and then porting it into the canonical design system with an exact no-regression gate. Invoke only when the user explicitly requests this skill or the complete two-pass fidelity-to-system process; do not use it for ordinary page building, visual review, or component cleanup.
---
...
2. `$averlo-next:design-system` before selecting or changing shared owners.
...
1. Inventory every temporary presentation owner and classify it using the
   placement rules in the reference contract.
...
3. Generalize shell contracts with explicit backward-compatible discriminators.
   Do not encode the source brand, page, or Figma frame in canonical APIs.
4. Add or update lowest-owner stories using literal story-level `tags` and
   `parameters.backport` metadata when the repo supports backports.
5. Remove the temporary fidelity scaffold only after its canonical replacement
   renders equivalently and is covered by the appropriate owner evidence.
6. Compare the canonical port against the frozen Pass A image. Acceptance is
...
Include the frozen checksum and source commit, owner-placement decisions,
stories and public APIs changed, breakpoint findings, and verification results.
Do not hand off or ship the Pass A scaffold as the finished implementation.

...
async function writeProjectDocs(
	...
	await fs.writeFile(
		...
		[
			...
			"- After the router is selected, do not separately invoke overlapping Averlo design-system, skeleton, entity, or surface skills for the same change unit. `$averlo:compose`, its peer planes (`$averlo:composition-realization`, `$averlo:composition-system-integration`, and `$averlo:composition-convergence`), `$averlo:motion-composition`, and `$averlo:visual-parity` are operational owners, not overlapping substitutes; they may invoke the repository router and Figma skills as subordinate steps. A source-backed composition must not call Figma or edit product code until Compose or the explicitly named peer plane is loaded and its preflight permits work.",

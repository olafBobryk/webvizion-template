...
hints. Apply the loaded Form protection concern before delivery work. Trim and
deduplicate configured recipients. Keep credentials server-only, return an
unavailable response when delivery is not configured, and expose a generic
recoverable failure without leaking provider details.
...
- Test server normalization, recipient deduplication, missing configuration,
  provider failure, and PII-safe logging. Use Form protection verification for
  guard behavior.

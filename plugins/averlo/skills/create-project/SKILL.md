...
3. Require `PRODUCT.md`, `docs/README.md`, and inherited `docs/operations/`
   when the selected profile includes an operational protocol. Confirm
   the retired `docs/project/` placeholder scaffold is absent.
...
For a lockfile-only project, put `npm install` before `npm run dev`. Use `$averlo:publish-project` only as a separate, explicitly requested external publishing workflow.

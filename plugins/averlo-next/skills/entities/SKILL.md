...
- `averlo-next:skeletons` for live/loading geometry and route skeletons;
- `averlo-next:surfaces` for route registries, navigation hierarchy, and contextual Command-K ownership;
- `averlo-next:figma-storybook-export` when entity owner stories or their exported Figma contract change;
- `code-clarity-cleanup` for barrels, direct imports, and boundary cleanup;
- `averlo-next:design-system` for shared component-policy compliance.
